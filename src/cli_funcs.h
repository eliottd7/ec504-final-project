...
--locker "path/to/locker"
    prints to the CLI: Contents: N files, XX.X KB/MB/GB used, {list of names of files}

--file "path/to/filename" (in addition to --locker)
    adds file to locker, also prints to the CLI locker data

    --save-as "/path/in/locker" (in addition to --file)
        changes how the file will be named in the locker

--delete "/path/in/locker" (in addition to --locker)
    removes the file from the locker, also prints to the CLI locker data
...
void add_file(string locker_path, string out_locker_path) {
    ...
    if ( !path_out_locker_path.exists() ) {
        string _err = "File or directory '" + out_locker_path + "' does not exist.";
        throw _err;
    }
    DDStore dd(locker_path.c_str());
...
// Adds file(s) into locker under the subfolder specified by in_locker_path
    ...
    if ( path_out_locker_path.is_directory() ) {
        for ( auto const& sub_path : fs::recursive_directory_iterator{path_out_locker_path} ) {
            if ( !sub_path.is_directory() ) { // this is string purgatory
                ...
                const fs::directory_entry path_locker_path(locker_path + "/" + in_locker_name.substr(0, in_locker_name.find_last_of("/")));
        ...
        const fs::directory_entry path_locker_path(locker_path + "/" + in_locker_path.substr(0, in_locker_path.find_last_of("/")));
        if ( !path_locker_path.exists() ) {
            fs::create_directories(path_locker_path.path());
        }
        dd.add_document(in_locker_path.c_str(), out_locker_path.c_str());
...
void delete_file(string locker_path, string in_locker_path) {
    if ( in_locker_path == "." ) { // just delete the whole locker
